...
installChaincodePeer1Org2(){
  ...
  echo
  echo "chaincode installed on peer1.org2.example.com"
  echo
}
...
insertTransaction(){
    ...
    echo "PEER1_ORG2_CA=${PEER1_ORG2_CA}"
    ...
    peer chaincode invoke -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C ${CHANNEL_NAME} -n ${CC_NAME} --peerAddresses localhost:10051 --tlsRootCertFiles $PEER1_ORG2_CA -c '{"function": "createCar", "Args":["CAR103","AudiX","R8","Black", "CM"]}'
...
readTransaction(){
    ...
    echo "channel name ${CHANNEL_NAME}"
    setGlobalsForPeer1Org2
    peer chaincode query -C $CHANNEL_NAME -n ${CC_NAME} --peerAddresses localhost:10051 --tlsRootCertFiles $PEER1_ORG2_CA  -c '{"function": "queryCar", "Args": ["CAR103"]}'
}


# installChaincodePeer1Org2CHANNEL_NAME
